	...
	while Q:
		...
		for dx, dy, dz in directions:
			...
			if x+dx not in range(X) or y+dy not in range(Y) or z+dz not in range(Z) or a+da not in [-1,0,1]:
				continue
			if (time+1, x+dx,y+dy,z+dz,a+da, hits+new_hits) not in visited:
				Q.append((time+1, x+dx,y+dy,z+dz,a+da, hits+new_hits))
				visited.add((time+1, x+dx,y+dy,z+dz,a+da, hits+new_hits))
		Q.append((time+1, x, y, z, a, hits+new_hits))